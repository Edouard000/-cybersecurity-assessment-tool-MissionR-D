...
""" HERE IS THE LAYOUT OF EVERYTHING RELATED TO THE CYBERSECURITY MATURITY DOMAIN 4 """
...
""" This class is responsible for the layout of the Cybersecurity Maturity's Domain 4 page """
class CSM_Domain4_Page(tk.Frame):
    #region
    def __init__(self, master):
        tk.Frame.__init__(self, master)
        master.title(login.Login_Page.logged_in.upper() + " - External Dependency Management")
        ...
        connections_button = tk.Button(self, text="Connections", font='Calibri 14', relief='groove', borderwidth=2, bg='light gray', activebackground='light blue',
                                       command=lambda: master.switch_frame(CSM_Domain4_Connections_Page))

        relationship_management_button = tk.Button(self, text="Relationship Management", font='Calibri 14', relief='groove', borderwidth=2, bg='light gray', activebackground='light blue',
                                                   command=lambda: master.switch_frame(CSM_Domain4_RelationshipManagement_Page))
        
        connections_label = tk.Label(self, font="Calibri 15", bg="ghost white",
                                     text=str(csm.submit_pressed(CSM_Domain4_Connections_Page.values)[3]) + "/" + str(sum([len(DATA.CSM_Domain4_Connections[i]) for i in DATA.CSM_Domain4_Connections])))
        
        relationship_management_label = tk.Label(self, font="Calibri 15", bg="ghost white",
                                                 text=str(csm.submit_pressed(CSM_Domain4_RelationshipManagement_Page.values)[3]) + "/" + str(sum([len(DATA.CSM_Domain4_RelationshipManagement[i]) for i in DATA.CSM_Domain4_RelationshipManagement])))
        ...
        reset_button = tk.Button(self, text='RESET', width=10, font='Calibri 14', relief='raised', borderwidth=2, bg='azure3', activebackground='light blue',
                                 command=lambda: CSM_Domain4_Page.reset(master))

        home_button.place(relx=0.1, rely=0.1)
        back_button.place(relx=0.25, rely=0.1)

        connections_button.place(relx=0.5, rely=0.4, anchor='center')
        relationship_management_button.place(relx=0.5, rely=0.5, anchor='center')

        connections_label.place(relx=0.67, rely=0.4, anchor='center')
        relationship_management_label.place(relx=0.67, rely=0.5, anchor='center')

        reset_button.place(relx=0.5, rely=0.8, anchor='center')

    # reset the answers of domain 4
    def reset(frame):
        confirm = messagebox.askokcancel('Confirmation', 'Are you sure you want to reset your answers ?')
        if confirm:
            csm.clear_pressed(CSM_Domain4_Connections_Page.values)
            csm.clear_pressed(CSM_Domain4_RelationshipManagement_Page.values)
            frame.switch_frame(CSM_Domain4_Page)
    #endregion


""" This class is responsible for the layout of the Cybersecurity Maturity's Domain 4 - Connections page """
class CSM_Domain4_Connections_Page(tk.Frame):
    ...
    def __init__(self, master):
        tk.Frame.__init__(self, master)
        master.title(login.Login_Page.logged_in.upper() + " - Connections")
        self.config(bg='ghost white')
        ...
        back_button = tk.Button(top_frame, text="BACK", width=10, font='Calibri 14', relief='raised', borderwidth=2, bg='azure3', activebackground='light blue',
                                command=lambda: master.switch_frame(CSM_Domain4_Page))
...
""" This class is responsible for the layout of the Cybersecurity Maturity's Domain 4 - Relationship Management page """
class CSM_Domain4_RelationshipManagement_Page(tk.Frame):
    ...
    def __init__(self, master):
        tk.Frame.__init__(self, master)
        master.title(login.Login_Page.logged_in.upper() + " - Relationship Management")
        self.config(bg='ghost white')
